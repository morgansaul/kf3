...
__global__ void KernelA(const TKparams Kparams)
{
    ...
    for (int step_ind = 0; step_ind < STEP_CNT; step_ind++)
    {
        ...
        for (int group = PNT_GROUP_CNT - 1; group >= 0; group--)
        {
			...
			// --- MILESTONE 2 (OPTIMIZED): MULTI-TARGET BINARY SEARCH ---
			if (Kparams.TargetCount > 0)
			{
				u64 left = 0;
				u64 right = Kparams.TargetCount - 1;
				while (left <= right)
				{
					u64 mid = left + (right - left) / 2;
					u64 mid_val = Kparams.d_Targets[mid * 8 + 0];

					if (x[0] == mid_val)
					{
						if (x[1] == Kparams.d_Targets[mid * 8 + 1] &&
							x[2] == Kparams.d_Targets[mid * 8 + 2] &&
							x[3] == Kparams.d_Targets[mid * 8 + 3])
						{
						...
						break; // Prevents infinite loop on partial collision
					}
					else if (x[0] < mid_val)
					{
